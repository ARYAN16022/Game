...
function setup() {
  ...
  castle = createSprite(700, 250);
  player = createSprite(windowWidth/2-150, 450);
...
function draw() {
  ...
  heading.position(windowWidth/2-500, windowHeight/2-200)

  scoreboard.html("Score:"+score)
  scoreboard.style('color:green')
  scoreboard.position(windowWidth/2+200, windowHeight/2-200)

  note.html("PRESS SPACE TO SHOOT BULLETS")
  note.style('color:brown')
  note.position(windowWidth/2+200, windowHeight/2-250)

  if (gameState===1) {
 player.y=mouseY

  for (let index = 0; index < windowWidth-100; index = index + 100) {
    b18 = createSprite(index, 800);
    b18.addImage(brick);
  ...
  for (let index = 120; index < 600; index = index + 100) {
    b18 = createSprite(index, 500);
    b18.addImage(brick);
        b21 = createSprite(600, index);
        b21.addImage(brick);
    b21.scale = 0.1;
    b18.scale = 0.1;
  }
...
for (let index = 0; index < windowHeight; index = index + 100) {
  b19 = createSprite(windowWidth-100, index);
  b19.addImage(brick);
  b19.scale = 0.1;
      b19 = createSprite(windowWidth - 1600, index);
      b19.addImage(brick);
      b19.scale = 0.1;
  }
